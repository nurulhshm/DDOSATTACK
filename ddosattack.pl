...
#Program to send out tcp syn packets using raw sockets in linux
...
sub main
{
	...
	#when IPROTO_RAW is used IP_HDRINCL is not needed
	$IPROTO_RAW=255;
	socket($sock, AF_INET, SOCK_RAW, $IPROTO_RAW)

	or die $!;

	#set IP_HDRINCL to 1, thus is necessary when the above protocol is something other than IPPROTO_RAW
...
sub makeheaders
{
	...
	my $tcp_header=pack('nnNNH2Bnvn' ,$src_port,$dst_port,$seq,$seq_ack,$tcp_doff_res,$tcp_flags,$tcp_win,$tcp_check,$tcp_urg_ptr);
	...
	#Function to calculate checksum-used in both ip and tcp headers sub checksum
{
	my ($msg)=@_;
